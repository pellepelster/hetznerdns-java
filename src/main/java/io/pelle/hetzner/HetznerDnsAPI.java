...
public class HetznerDnsAPI {
  ...
  public ZoneResponse updateZone(String zoneId, ZoneRequest request) {
    var entity = new HttpEntity<>(request, httpHeaders);
    return request("/zones/" + zoneId, HttpMethod.PUT, entity, ZoneResponseWrapper.class)
        .map(ZoneResponseWrapper::getZone)
        .orElse(null);
  }
  ...
  public List<RecordResponse> createRecords(RecordRequest... request) {
    var entity =
        new HttpEntity<>(
